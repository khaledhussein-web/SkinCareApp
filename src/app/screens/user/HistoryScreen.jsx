...
export const HistoryScreen = () => {
  ...
  useEffect(() => {
    // Fetch the authenticated user's assessment history on screen load.
    const loadHistory = async () => {
      if (!user?.id) return;
      setLoading(true);
      setError("");
      try {
        const response = await fetchAssessmentHistory();
        setAssessments(response.history || []);
      } catch (historyError) {
  ...
  const getScoreColor = (score) => {
    // Visual score classification for quick status scanning.
  ...
  const progressStats = useMemo(() => {
    if (assessments.length === 0) {
      return { latestScore: 0, improvement: 0, total: 0 };
    }
    const latestScore = assessments[0]?.score || 0;
    const oldestScore = assessments[assessments.length - 1]?.score || 0;
    return {
      latestScore,
  ...
  const openAssessment = (assessment) => {
    // Rehydrate selected historical assessment into shared context and open results view.
            ...
            <CardContent className="p-6 sm:p-8">
              ...
              <div className="grid sm:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <p className="text-4xl">{progressStats.improvement >= 0 ? `+${progressStats.improvement}` : progressStats.improvement}</p>
                </div>
                <div>
                  <p className="text-white/80 text-sm mb-1">Total Assessments</p>
                  <p className="text-4xl">{progressStats.total}</p>
                </div>
              </div>
            </CardContent>
          </Card>
